...
- 我想了解每个区域做什么：见 [页面分区与职责](#页面分区与职责)
- 我想找顶栏功能：见 [顶栏 Menu](#顶栏-menu)
- 我想看各模块如何协同：见 [模块联动](./workflow.md)
- 我想查图标：见 [附录：按钮速查表](../appendix/buttons.md)

## 页面分区与职责

- 左侧：`时间表 / 区块编辑器`，负责当天可用时间与任务占位，详见 [时间表构建](timetable.md)。
- 中上：`计划表`，负责日/周/月/年排程与执行状态，详见 [计划表](planner.md)。
- 中下：`任务追踪`，负责执行过程记录（能量/愉悦/打扰/文本），详见 [任务追踪](task.md)。
- 右侧：`活动清单`，负责活动收集、筛选与看板整理，详见 [活动清单](activity.md)。
- 浮动区：`番茄意图计时器`，负责专注和休息节奏，详见 [番茄意图计时器](timer.md)。
...
- `首页`：进入主工作区。
- `数据页`：查看和检索历史数据，详见 [数据查看](search.md)。
- `仪表盘`：查看统计趋势，详见 [数据趋势](./chart.md)。
- `帮助页`：查看软件帮助信息。
- `设置页`：查看状态与调试工具。
- <img src="/icons/List24Filled.svg" width="20" style="display:inline-block;vertical-align:middle;margin:0;background:rgb(244, 244, 244);border-radius: 6px;">：手机端收缩菜单按钮。
...
|    <img src="/icons/TasksApp20Regular.svg" width="20" style="display:inline-block;vertical-align:middle;margin:0;background:rgb(255, 255, 255);border-radius: 6px;">    | 切换计划表视图 `Planner`               |
...
- 这里关注“界面分区和入口”；跨模块的数据流和生命周期说明见 [模块联动](./workflow.md)。
