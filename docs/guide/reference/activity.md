...
- 我想了解活动类型：见 [活动类型](#活动类型)
- 我想新建、删除、复制活动：见 [通用操作](#通用操作)
- 我想调整层级或切换待办子类型：见 [待办 Todo 专属操作](#待办-todo-专属操作)
- 我想筛选、排序、管理看板列：见 [视图与筛选](#视图与筛选)
- 我想查看颜色含义：见 [视觉规则](#视觉规则)
- 我想查图标含义：见 [附录：按钮一览](../appendix/buttons.md#活动清单工具栏)
...
| 图标                                                                                                                                                                       | 名称     | 英语                    | 定义                                                                                                                                                           |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <img src="/icons/ApprovalsApp24Regular.svg" width="20" style="display:inline-block;vertical-align:middle;margin:0;background:rgb(224, 224, 224);border-radius: 6px;">      | 待办     | `Todo`                  | 以原子时间块拆分但不一定有固定截止日期的任务。分三种子类型：<li>🍅 分配为工作的 `25min`</li><li>🍒 分配为工作的 `4×15min`</li><li>🍇 分配到生活的 `25min`</li> |
...
| <img src="/icons/Cloud24Regular.svg" width="20" style="display:inline-block;vertical-align:middle;margin:0;background:rgb(224, 224, 224);border-radius: 6px;">             | 无所事事 | `Untaetigkeit`          | 与自己约定留白或发呆的时间段。                                                                                                                                 |
...
以下操作适用于 `Todo`、`Schedule`、`Untaetigkeit`（未特别说明时）：

- **新建**
  - `Todo`：点击 <img src="/icons/AddCircle24Regular.svg" width="20" style="display:inline-block;vertical-align:middle;margin:0; background:rgb(193, 226, 255);border-radius: 6px;">
  - `Schedule`：点击 <img src="/icons/CalendarAdd24Regular.svg" width="20" style="display:inline-block;vertical-align:middle;margin:0;background:rgb(193, 226, 255);border-radius: 6px;">
  - `Untaetigkeit`：点击 <img src="/icons/CloudAdd20Regular.svg" width="20" style="display:inline-block;vertical-align:middle;margin:0;background:rgb(193, 226, 255);border-radius: 6px;">，新增后自动添加到当前时间
...
- **生成子活动**：选中活动，点击二级菜单中的 <img src="/icons/TextGrammarArrowRight24Regular.svg" width="20" style="display:inline-block;vertical-align:middle;margin:0;background:rgb(224, 224, 224);border-radius: 6px;">，仅支持一级子活动
- **升级为兄弟**：选中子活动，点击二级菜单中的 <img src="/icons/TextGrammarArrowLeft24Regular.svg" width="20" style="display:inline-block;vertical-align:middle;margin:0;background:#d9fccc;border-radius: 6px;">
- **修改子类型**：双击输入预估番茄数输入框， 按顺序切换（🍅/🍒/🍇）；切换后预估番茄数清零，🍒 默认 4 个

## 视图与筛选

- **点击筛选**：点击 <img src="/icons/DocumentTableSearch24Regular.svg" width="20" style="display:inline-block;vertical-align:middle;margin:0;background:rgb(224, 224, 224);border-radius: 6px;">，在下拉菜单选择预设选项后完成筛选
- **录入筛选**：输入关键词进行筛选。预设选项的中英文关键词：
  - 全部活动 `all`
  - 今日到期 `today`
  - 内外打扰 `interrupt`
  - 待办活动 `todo`
