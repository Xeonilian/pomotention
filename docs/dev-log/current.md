...
| **分支** | `dev` |
| **更新** | 2026-07-31 |
| **停在哪** | 记一句第一刀代码已接上：右栏 `CapturePanel` + `runCapture` → Worker → create todo。请本地验：`pnpm gateway:dev` + `pnpm dev`，工具栏点「记一句」，试「明天下午写周报，两个番茄」。 |
...
- [x] **1.** 记一句专用面（`CapturePanel`，工具栏可开关）  
- [x] **2.** capture → todo Writer（`src/core/capture/`）  
- [ ] **3.** 联调演示（待你本地验）  
