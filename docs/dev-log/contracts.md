...
## Contract #7: [S2 C1 UV1] 数据页面右侧内容“加星”显示与编辑
...
## Contract #8: [S2 C1 UV1] 数据页面“标签”显示、编辑与筛选
...
## Contract #10: [S2 C1 UV1] 注册验证并重新登录

### Given:

- 应用版本 ≥ v0.4.4
- 用户从未注册过（邮箱未在 Supabase 中）
- 应用已启动并显示登录页面
...
1. 页面显示提示：`"注册成功！请检查您的邮箱以完成验证，然后尝试登录。"`
2. 用户收到 Supabase 发送的验证邮件
3. 用户在邮件中点击验证链接
4. 浏览器自动打开应用并跳转到登录页
5. 控制台显示：`"检测到邮箱验证，正在处理..."`
6. 控制台显示：`"验证成功！"`
7. 自动跳转到主页面
8. 用户已登录状态（session 已建立）
...
- ⚠️ **当前行为（需：** 显示"注册成功"但不发邮件
- ✅ **期望行为：** 显示错误提示 `"该邮箱已注册，请直接登录"`
