...
### 3.2 [`DayTodo.vue`](https://github.com/Xeonilian/pomotention/blob/main/src/components/DayPlanner/DayTodo.vue)
...
### 3.3 [`DaySchedule.vue`](https://github.com/Xeonilian/pomotention/blob/main/src/components/DayPlanner/DaySchedule.vue)
...
### 3.4 [`ActivitySection.vue`](https://github.com/Xeonilian/pomotention/blob/main/src/components/ActivitySheet/ActivitySection.vue)
...
### 3.5 [`useLongPress.ts`](https://github.com/Xeonilian/pomotention/blob/main/src/composables/useLongPress.ts)
...
| 文件                                                                                                                           | 作用（一行）                                                                                          |
| ------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------- |
| [`useDraggable.ts`](https://github.com/Xeonilian/pomotention/blob/main/src/composables/useDraggable.ts)                        | `@pointerdown` → `handleDragStart`；`preventDefault` + 阈值 5px 拖拽（迷你浮窗等）                    |
| [`MainLayout.vue`](https://github.com/Xeonilian/pomotention/blob/main/src/views/MainLayout.vue) L106                           | 布局上拖拽柄：`@pointerdown="handleDragStart"`，使用 `useDraggable(5)`                                |
| [`useActivityDrag.ts`](https://github.com/Xeonilian/pomotention/blob/main/src/composables/useActivityDrag.ts)                  | Activity 列表拖拽；模板约定 `@pointerdown="startDrag($event, item)"`                                  |
| [`useResize.ts`](https://github.com/Xeonilian/pomotention/blob/main/src/composables/useResize.ts)                              | `startResize`：分隔条拖动调整高度/宽度                                                                |
| [`HomeView.vue`](https://github.com/Xeonilian/pomotention/blob/main/src/views/HomeView.vue) L19 / L280 / L295                  | 左/中/右分隔：`@pointerdown` → `startLeftResize` / `startVerticalResize` / `startRightResize`         |
| [`SearchView.vue`](https://github.com/Xeonilian/pomotention/blob/main/src/views/SearchView.vue) L114                           | 非移动：`resize-handle-horizontal` + `touch-action: none` + `@pointerdown="resizeSearch.startResize"` |
| [`TimeBlocks.vue`](https://github.com/Xeonilian/pomotention/blob/main/src/components/TimeTable/TimeBlocks.vue) L53 / L70       | Todo 段：`@pointerdown="enhancedHandlePointerDown"`（逻辑在 `useTimeBlocks`：选中/拖拽时间条等）      |
| [`PomodoroTimer.vue`](https://github.com/Xeonilian/pomotention/blob/main/src/components/PomotentionTimer/PomodoroTimer.vue) L4 | 可编辑文案：`@click.stop` + `@pointerdown.stop`，避免父级抢事件                                       |

## 5. 全局触摸辅助

| 文件                                                                                                   | 作用                                                                                                                         |
| ------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| [`touchHandler.ts`](https://github.com/Xeonilian/pomotention/blob/main/src/core/utils/touchHandler.ts) | `document`：`touchstart`/`touchmove`/`touchend`；非交互元素横向快滑时 `touchmove` 上 `preventDefault`（与 **F** 同类但全局） |
...
_文档版本：与仓库第一步梳理同步；行号若因编辑漂移请以 IDE 为准。_
