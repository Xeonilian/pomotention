...
| 链路               | 含义                                         |  MCP   |
| ------------------ | -------------------------------------------- | :----: |
| **P context**      | 上下文出口：结构化摘要导出给外部 AI          |   否   |
| **User Agent App** | 工具层：AI 读写 Pomotention（MCP Server 等） |   是   |
| **P info**         | 觉察卡片：App 内展示 AI 写回（非聊天框）     | 依赖 C |
...
localStorage 已偏紧；chart 与 A 出口依赖 **稳定可聚合** 的本地读模型。
...
Pomotention **不做**完整记账 App：在 App 里快记，在外部 plaintext 账本（beancount、hledger 等）入账。

**边界：** 不做余额、报表、多账套；不做 Beancount/Fava 替代；合规与税表不在范围。

**可能步骤（顺序未钉死）：**

- 文本 → ledger 行识别（beancount 或 hledger 二选一）— **current 队列 #1**
- 一键复制 ledger 片段
- Task 内 code block / 专用模板（可第二版）
...
基于文档的问答引导 — 与「主力 AI + A 出口」不同线；依赖帮助正文质量。
