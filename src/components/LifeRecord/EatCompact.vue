    ...
    <div class="lr-header">
      <div class="lr-header__lead">
        <n-button text size="small" class="lr-icon-btn" :title="`记一条${def.title}`" @click="append">
          <template #icon>
            <n-icon :size="20"><FoodApple20Regular /></n-icon>
          </template>
        </n-button>
...
import { Delete20Regular, Dismiss20Regular, FoodApple20Regular } from "@vicons/fluent";
import { useLifeRecordEditor } from "@/composables/lifeRecord/useLifeRecordEditor";
import EatTag from "@/components/LifeRecord/EatTag.vue";
