...
const mobilePinnedIds = computed(() => normalizeTaskToolbarMobilePinned(settingStore.settings.taskToolbarMobilePinned));
...
function handleMoreClick() {
  ...
  if (editSelection.value.length > 0) {
    settingStore.settings.taskToolbarMobilePinned = mergeTaskToolbarMobilePinned(mobilePinnedIds.value, editSelection.value);
  }
  popoverEditMode.value = false;
  editSelection.value = [];
  showCollapsedPopover.value = false;
