    ...
    <n-card size="small" class="settings-diagnostics-card" :bordered="true">
      <n-collapse :default-expanded-names="[]" display-directive="show">
        ...
        <n-collapse-item title="视口与 safe-area（布局调试）" name="viewport">
          ...
          <n-descriptions v-if="viewportSnapshot" label-placement="left" :column="1" bordered size="small" style="margin-bottom: 12px">
            ...
            <n-descriptions-item label="safe-area（解析）">
              t {{ viewportSnapshot.safeArea.top }} · r {{ viewportSnapshot.safeArea.right }} · b {{ viewportSnapshot.safeArea.bottom }} · l
              {{ viewportSnapshot.safeArea.left }}
            </n-descriptions-item>
        ...
        <n-collapse-item title="环境诊断" name="env">
          ...
          <n-space style="margin-bottom: 8px">
            <n-button v-if="showExitLocalOnly" size="small" type="primary" :loading="exitLocalOnlyLoading" @click="handleExitLocalOnlyMode">
              启用云同步（关闭仅本地）
            </n-button>
        ...
        <n-collapse-item title="音频诊断（iOS / PWA）" name="audio">
          <p class="audio-dbg-hint">
            记录提示音与白噪音的关键事件。白噪音为 HTML 双轨 crossfade；iOS 在 AudioContext 已 running 时定时提示音多走 Web Audio，否则仍以
            HTML 为主。仅内存，刷新清空。出现
            <code>[cue] FAIL</code>
            或
...
import { clearLocalData } from "@/services/data/downloadService";
...
import { useViewportDebugSnapshot } from "@/composables/layout/useViewportDebugSnapshot";
