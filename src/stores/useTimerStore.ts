...
export const useTimerStore = defineStore('timer', () => {  
  ...
  const redBarOffsetPercentage = ref<number>(r1Duration.value/workDuration.value)
  const redBarPercentage = ref<number>(wDuration.value*2/workDuration.value)
  ...
  // 方法  
  function startWorking(duration: number): void {  
  ...
  function startBreak(duration: number): void {  
    if (timerInterval.value) clearInterval(timerInterval.value);  
  ...
  function cancelTimer(): void {  // 重命名为cancelTimer更直观  
    resetTimer();  
  }  
