<template>  
  <div class="pomodoro-timer">  
    ...
    <!-- 3 工作进度条 -->  
    <!-- 3-1 进度条容器 -->  
    <div class="progress-container" v-if="!timerStore.isBreaking" :style="timerStyleVars">  
      <!-- 3-2 蓝色进度条 -->  
      <n-progress  
...
<script setup lang="ts">  
  ...
  // 把常量转成 CSS 变量名格式  
    ...
    '--red-bar-length': `${parseFloat(barLength.value) * timerStore.redBarPercentage}px`,  
    '--red-bar-offset': `${parseFloat(barLength.value) * timerStore.redBarOffsetPercentage}px`,  
  }))  
  ...
  const currentPhase = computed((): 'r1' | 'w1' | 'w2' | 'r2' | 't' => {  
    ...
    const r2End = (r1Duration+2*wDuration+r2Duration) / workDuration * 100;
...
<style>  
  ...
  /* 3-3-3  设置分隔线位置 */  
  .phase-labels .divider:nth-child(2) { left: 8%; }  
  .phase-labels .divider:nth-child(4) { left: 50%; }  
  .phase-labels .divider:nth-child(6) { left: 92%; }  
  .phase-labels .divider:nth-child(8) { left: 96%; }  
    

  /* 重新计算每个元素的宽度 */  
  .phase-labels .phase-label:nth-child(1) { width: 8%; }   /* r: 2/25 ≈ 8% */  
  .phase-labels .phase-label:nth-child(3) { width: 42%; }  /* w: 10.5/25 ≈ 42% */  
  .phase-labels .phase-label:nth-child(5) { width: 42%; }  /* w: 10.5/25 ≈ 42% */  
  .phase-labels .phase-label:nth-child(7) { width: 4%; }   /* r: 1/25 ≈ 4% */  
  .phase-labels .phase-label:nth-child(9) { width: 4%; }   /* t: 1/25 ≈ 4% */    

  /* 调整分隔符位置，确保恰好位于边界上 */  
  .phase-labels .divider:nth-child(2) { position: absolute; left: 8%; transform: translateX(-50%); }  
  .phase-labels .divider:nth-child(4) { position: absolute; left: 50%; transform: translateX(-50%); }  
  .phase-labels .divider:nth-child(6) { position: absolute; left: 92%; transform: translateX(-50%); }  
  .phase-labels .divider:nth-child(8) { position: absolute; left: 96%; transform: translateX(-50%); }
